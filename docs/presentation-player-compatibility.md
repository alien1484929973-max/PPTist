...
- 编辑器放映菜单的“依赖播放器预览（Beta）”使用 `createPresentationPlayer` 播放当前 Pinia 文稿。
...
`analyzePresentationCompatibility(document)` 会在正式切换前扫描真实文稿：未知元素是 blocking；未知导入转场使用可靠淡入回退，无法映射的旧元素动画落到确定终态，两者都会给出 warning。图表必须由包内 ECharts SVG 渲染器输出，不允许占位符通过门槛。
