...
演讲者备注折叠栏可点击展开并聚焦；备注编辑器按页面 ID 隔离，内容事务立即写入
Pinia action，并按现有防抖规则记录快照，避免快速切页丢字或串页。外部内容更新需要同步，
但不能把同步本身作为用户编辑再次保存。

文本框的文字区域优先使用原生光标和拖选，不放置覆盖文字的透明拖动层。
选中单个文本框或组合中的激活文本后，框外左侧显示固定屏幕尺寸的移动手柄，
按住即可拖动；通过画布原有 `dragElement` 保留吸附、Ctrl 复制、Shift 方向约束和快照。
左键静止按住 450ms 后可拖动整个文本框；提前移动超过 5px、松开或窗口失焦取消等待。
内边距仍可直接拖动，Ctrl/Shift 组合选择、缩放和旋转沿用画布现有逻辑。
...
擦除动画的 clip 必须按目标 SVG 描边和线端 marker 留出外扩余量，不能用元素边界上的
`inset(0)`，否则箭头尖角和粗描边会在播放过程中被切成半边。
...
PPTX 箭头 preset 的 `adj1` / `adj2` 必须结合形状宽高换算为编辑器的杆身粗细和箭头长度；
不能只保留一个调整值，否则导入后虽然仍是箭头类型，几何外观和后续编辑都会失真。
...
- `definePresentationWidget()` / `inspectPresentationRequirements()`；
- 时间线、图表、图片和资源辅助 API。
