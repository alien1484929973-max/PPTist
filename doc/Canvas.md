...
点击文本直接定位光标，立即拖动可选择文字。选中文本框后，框外左侧出现四向箭头
移动手柄，按住即可直接拖动，不需要等待。手柄大小不随画布缩放变小，
并支持 Ctrl 拖动复制、Shift 拖动约束方向及撤销。

在文字上静止按住左键约 450ms 后，
光标变为移动样式，再拖动可移动文本框。文本框内边距也可直接拖动。
上下边缘不再覆盖透明拖动层，以保证小字号和单行文本可点击。

底部“演讲者备注”可点击展开并编辑已有内容，也可拖动顶部边缘调整高度。
备注输入即时同步到当前页，支持格式编辑和撤销；快速切页不会把未同步的文字写入其他页。
