...
当前版本：`0.2.0`。该稳定版在 PPTist 自身的桌面放映、演讲者视图、观众窗口和移动播放路径中实际调用；编辑器也从包的 `exports` 加载构建后的 `dist`，不再使用旧 Vue 播放回退。
...
```bash
npm install pptist-presentation-player@0.2.0
```

也可以从官网下载离线包，校验 `SHA256SUMS.txt` 后从本地安装：

```bash
npm install ./pptist-presentation-player-0.2.0.tgz
```
